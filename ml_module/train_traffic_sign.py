...
def load_and_preprocess(data_path, limit_per_class=None, log_func=print):
    log_func(f"1 [LOAD] Loading from: {data_path}")
...
def start_training(data_path, limit=50, epochs=5, log_func=print):
    try:
        ...
        log_func("2 [MODEL] Constructing CNN Architecture...")
        model = build_model(X_train.shape[1:], 43)

        log_func(f"3 [TRAIN] Running {epochs} Epochs on {len(X_train)} samples...")
        ...
        log_func(" Model Saved to 'models/traffic_classifier.h5'")
